...
class SVM_HMM_Weight(object):
    def __init__(self, num_dims=None, num_labels=None, weight_name=None, init_zero_weights = False):
        if weight_name is not None:
            self.feature_weights, self.time_weights, self.start_time_weights, self.bias = self.load_weights(weight_name)
            return
    ...
    def load_weights(self, weight_name):
        ...
        start_time_weights = weight_dict['start_time_weights']
        bias = weight_dict['bias']
        return weight_dict['feature_weights'], weight_dict['time_weights'], np.reshape(start_time_weights, (start_time_weights.size,)), np.reshape(bias, (bias.size,))

    def save_weights(self, weight_name):
        sp.savemat(weight_name, {'feature_weights' : self.feature_weights, 'time_weights' : self.time_weights, 'start_time_weights' : self.start_time_weights, 'bias' : self.bias}, oned_as='column')

    def norm(self):
        return np.sqrt(np.sum(self.feature_weights ** 2) + np.sum(self.time_weights ** 2) + np.sum(self.start_time_weights ** 2) + np.sum(self.bias ** 2))
