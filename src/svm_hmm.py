...
class SVM_HMM_Weight(object):
    ...
    def load_weights(self, weight_name):
        ...
        return weight_dict['feature_weights'], weight_dict['time_weights']
...
#                print "projecting"
    ...
    def parallel_time_forward_chunk(self, feature_chunk, labels = None):
        max_sequence_len = feature_chunk.shape[0]
        forward_chunk = np.zeros(feature_chunk.shape)
        argmax_chunk = np.zeros(feature_chunk.shape)
        forward_chunk[:,:,0] = feature_chunk[:,:,0]
        for sequence_index in range(1,max_sequence_len):
            time_i_trellis = forward_chunk[:,:,sequence_index-1] * self.weights.time_weights * feature_chunk[:,:,sequence_index]
